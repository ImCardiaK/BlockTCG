...
public final class DuelManager {
    ...
    private List<String> activeDeck(Player p) {
        ...
        return deck != null && deck.size() == size ? deck : null;
    }
    ...
    private boolean checkEnd(Duel duel) {
        ...
        if (!winner.npc) {
            ...
            if (wp != null) {
                int reward = loser.npc ? plugin.getConfig().getInt("game.npc-reward-coins",75) : plugin.getConfig().getInt("game.player-win-reward-coins",25);
