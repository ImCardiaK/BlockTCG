...
public final class MenuManager {
    ...
    public static final String MERCHANT_BOOSTER_TITLE = ChatColor.GOLD + "BlockTCG • Marchand • Boosters";
    public static final String DECK_SHOP_TITLE = ChatColor.DARK_AQUA + "BlockTCG • Vendeur de decks";
    public static final String TRADE_TITLE = ChatColor.DARK_GREEN + "BlockTCG • Échange";
    public static final String TRADE_CARDS_TITLE = ChatColor.DARK_GREEN + "BlockTCG • Cartes échange";
    ...
    public void openMerchantBuy(Player p, int page) {
        List<CardDefinition> cards = plugin.npcs().currentStock(p);
        int maxPage = Math.max(0, (cards.size() - 1) / 45);
        page = Math.max(0, Math.min(page, maxPage));
        ...
        for (int i = 0; i < 45; i++) {
            ...
            CardDefinition card = cards.get(idx);
            ItemStack item = plugin.cardItems().createForViewer(card, 1, p);
            addLore(item, "", ChatColor.GOLD + plugin.lang().tr("merchant.buy-price", "price", plugin.npcs().buyPrice(card)), ChatColor.GREEN + plugin.lang().tr("merchant.click-buy"));
            inv.setItem(i, item);
        }
        if (page > 0) inv.setItem(45, simple(Material.ARROW, ChatColor.YELLOW + plugin.lang().tr("menu.previous"), "merchant-buy|" + (page - 1)));
        inv.setItem(47, simple(Material.BUNDLE, ChatColor.LIGHT_PURPLE + plugin.lang().tr("merchant.boosters"), "merchant-boosters"));
        inv.setItem(48, simple(Material.EMERALD, ChatColor.GREEN + plugin.lang().tr("merchant.sell-mode"), "merchant-sell|0"));
        inv.setItem(49, simple(Material.GOLD_NUGGET, ChatColor.GOLD + plugin.lang().tr("menu.coins", "coins", plugin.data().get(p.getUniqueId()).coins()), null));
        inv.setItem(50, simple(Material.CLOCK, ChatColor.AQUA + plugin.lang().tr("merchant.rotation", "seconds", plugin.npcs().secondsUntilRotation()), null));
        if (page < maxPage) inv.setItem(53, simple(Material.ARROW, ChatColor.YELLOW + plugin.lang().tr("menu.next"), "merchant-buy|" + (page + 1)));
        p.openInventory(inv);
    }
    ...
    public void openMerchantBoosters(Player p) {
        Inventory inv = Bukkit.createInventory(null, 27, MERCHANT_BOOSTER_TITLE);
        int slot = 10;
        for (BoosterDefinition booster : plugin.boosters().all()) {
            if (slot > 16) break;
            ItemStack item = plugin.boosterManager().create(booster, 1);
            addLore(item, "", ChatColor.GOLD + plugin.lang().tr("merchant.buy-price", "price", booster.price()), ChatColor.GREEN + plugin.lang().tr("merchant.click-buy"));
            ItemMeta meta=item.getItemMeta(); meta.getPersistentDataContainer().set(actionKey, PersistentDataType.STRING, "buy-booster|"+booster.id()); item.setItemMeta(meta);
            inv.setItem(slot++, item);
        }
        inv.setItem(22, simple(Material.ARROW, ChatColor.YELLOW + plugin.lang().tr("menu.back"), "merchant-buy|0"));
        p.openInventory(inv);
    }

    public void openPrebuiltDeckShop(Player p) {
        Inventory inv = Bukkit.createInventory(null, 54, DECK_SHOP_TITLE);
        int slot=0;
        for (PrebuiltDeck deck : plugin.prebuiltDecks().all()) {
            Material mat=Material.matchMaterial(deck.container()); if(mat==null)mat=Material.CHEST;
            ItemStack item=simple(mat, ChatColor.AQUA+deck.name(), "buy-prebuilt|"+deck.id());
            addLore(item, ChatColor.GRAY+plugin.lang().tr("deckshop.theme", "theme", deck.theme()), ChatColor.GRAY+plugin.lang().tr("deckshop.cards", "count", deck.cards().size()), ChatColor.GOLD+plugin.lang().tr("merchant.buy-price", "price", deck.price()), "", ChatColor.GREEN+plugin.lang().tr("merchant.click-buy"));
            inv.setItem(slot++,item); if(slot>=45)break;
        }
        inv.setItem(49,simple(Material.GOLD_NUGGET,ChatColor.GOLD+plugin.lang().tr("menu.coins","coins",plugin.data().get(p.getUniqueId()).coins()),null));
        inv.setItem(53,simple(Material.BARRIER,ChatColor.RED+plugin.lang().tr("menu.close"),"close"));
        p.openInventory(inv);
    }

    public void openTrade(Player p) {
        TradeManager.Offer own=plugin.trades().offer(p.getUniqueId()); if(own==null)return;
        TradeManager.Offer other=plugin.trades().partnerOffer(p.getUniqueId());
        Player partner=Bukkit.getPlayer(own.partner);
        Inventory inv=Bukkit.createInventory(null,54,TRADE_TITLE+" • "+(partner==null?"?":partner.getName()));
        int slot=0;
        for(var e:own.cards.entrySet()){ if(slot>=18)break; CardDefinition c=plugin.cards().get(e.getKey()); if(c!=null){ItemStack it=plugin.cardItems().createForViewer(c,1,p);addLore(it,ChatColor.YELLOW+"× "+e.getValue());inv.setItem(slot++,it);} }
        slot=27;
        if(other!=null)for(var e:other.cards.entrySet()){ if(slot>=45)break; CardDefinition c=plugin.cards().get(e.getKey()); if(c!=null){ItemStack it=plugin.cardItems().createForViewer(c,1,p);addLore(it,ChatColor.AQUA+"× "+e.getValue());inv.setItem(slot++,it);} }
        inv.setItem(18,simple(Material.LIME_STAINED_GLASS_PANE,ChatColor.GREEN+plugin.lang().tr("trade.your-offer"),null));
        inv.setItem(26,simple(Material.CYAN_STAINED_GLASS_PANE,ChatColor.AQUA+plugin.lang().tr("trade.their-offer"),null));
        inv.setItem(45,simple(Material.BOOK,ChatColor.YELLOW+plugin.lang().tr("trade.manage-cards"),"trade-cards"));
        inv.setItem(46,simple(Material.RED_DYE,ChatColor.RED+"-100", "trade-coins|-100"));
        inv.setItem(47,simple(Material.RED_DYE,ChatColor.RED+"-10", "trade-coins|-10"));
        inv.setItem(48,simple(Material.GOLD_NUGGET,ChatColor.GOLD+plugin.lang().tr("trade.coins", "coins", own.coins),null));
        inv.setItem(49,simple(Material.LIME_DYE,ChatColor.GREEN+"+10", "trade-coins|10"));
        inv.setItem(50,simple(Material.LIME_DYE,ChatColor.GREEN+"+100", "trade-coins|100"));
        inv.setItem(51,simple(Material.SHULKER_BOX,ChatColor.LIGHT_PURPLE+plugin.lang().tr("trade.deck", "deck", own.deckName==null?plugin.lang().tr("trade.none"):own.deckName),"trade-deck"));
        inv.setItem(52,simple(own.confirmed?Material.LIME_CONCRETE:Material.YELLOW_CONCRETE,own.confirmed?ChatColor.GREEN+plugin.lang().tr("trade.confirmed"):ChatColor.YELLOW+plugin.lang().tr("trade.confirm"),"trade-confirm"));
        inv.setItem(53,simple(Material.BARRIER,ChatColor.RED+plugin.lang().tr("trade.cancel"),"trade-cancel"));
        if(other!=null){ addLore(inv.getItem(26),ChatColor.GRAY+plugin.lang().tr("trade.coins","coins",other.coins),ChatColor.GRAY+plugin.lang().tr("trade.deck","deck",other.deckName==null?plugin.lang().tr("trade.none"):other.deckName),other.confirmed?ChatColor.GREEN+plugin.lang().tr("trade.confirmed"):ChatColor.YELLOW+plugin.lang().tr("trade.waiting")); }
        p.openInventory(inv);
    }

    public void openTradeCards(Player p, int page) {
        TradeManager.Offer own=plugin.trades().offer(p.getUniqueId()); if(own==null)return;
        PlayerData data=plugin.data().get(p.getUniqueId());
        List<CardDefinition> cards=plugin.cards().list().stream().filter(c->data.cardCount(c.id())>0).toList();
        int maxPage=Math.max(0,(cards.size()-1)/45);page=Math.max(0,Math.min(page,maxPage));
        Inventory inv=Bukkit.createInventory(null,54,TRADE_CARDS_TITLE+" • "+(page+1));
        for(int i=0;i<45;i++){int idx=page*45+i;if(idx>=cards.size())break;CardDefinition c=cards.get(idx);ItemStack it=plugin.cardItems().createForViewer(c,1,p);addLore(it,"",ChatColor.GRAY+plugin.lang().tr("trade.owned","count",data.cardCount(c.id())),ChatColor.YELLOW+plugin.lang().tr("trade.offered","count",own.cards.getOrDefault(c.id(),0)),ChatColor.GREEN+plugin.lang().tr("trade.left-add"),ChatColor.RED+plugin.lang().tr("trade.right-remove"));inv.setItem(i,it);}
        if(page>0)inv.setItem(45,simple(Material.ARROW,ChatColor.YELLOW+plugin.lang().tr("menu.previous"),"trade-card-page|"+(page-1)));
        inv.setItem(49,simple(Material.ARROW,ChatColor.YELLOW+plugin.lang().tr("menu.back"),"trade-back"));
        if(page<maxPage)inv.setItem(53,simple(Material.ARROW,ChatColor.YELLOW+plugin.lang().tr("menu.next"),"trade-card-page|"+(page+1)));
        p.openInventory(inv);
    }
