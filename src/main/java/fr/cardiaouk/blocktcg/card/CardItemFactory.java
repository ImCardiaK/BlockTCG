...
public final class CardItemFactory {
    ...
    private ItemStack create(CardDefinition card, int amount, boolean revealId) {
        ...
        lore.add(ChatColor.GRAY + plugin.lang().tr("card.theme") + "  " + ChatColor.WHITE + "◆ " + card.theme());
        lore.add(ChatColor.GRAY + plugin.lang().tr("card.rarity") + " " + card.rarity().color() + "◆ " + prettyRarity(card.rarity()));
        lore.add(ChatColor.DARK_GRAY + "━━━━━━━━━━━━━━━━━━━━");
        lore.add(ChatColor.RED + "⚔ " + plugin.lang().tr("card.attack") + "  " + ChatColor.WHITE + card.attack());
        lore.add(ChatColor.BLUE + "🛡 " + plugin.lang().tr("card.defense") + "  " + ChatColor.WHITE + card.defense());
        lore.add(ChatColor.AQUA + "✧ " + plugin.lang().tr("card.mana") + "     " + ChatColor.WHITE + card.mana());
        if (!card.ability().isBlank()) {
            lore.add("");
            lore.add(ChatColor.GOLD + "✦ " + plugin.lang().tr("card.effect"));
            lore.add(ChatColor.YELLOW + "  " + card.ability());
        }
        if (revealId) {
            lore.add("");
            lore.add(ChatColor.DARK_GRAY + plugin.lang().tr("card.admin-id") + ": " + card.id());
        }
        lore.add(ChatColor.DARK_GRAY + "━━━━━━━━━━━━━━━━━━━━");
        meta.setLore(lore);
